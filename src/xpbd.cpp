...
class CApplication {
private:
	float m_Time;         // 仿真时间（虚拟世界中流逝的时间）
	int   m_SolveTime;    // 单步解算时间（现实世界中流逝的时间）
	long  m_frames;       // 自程序开始时，累计绘制的帧数
public:
	int   m_IterationNum;
	int   m_Mode;
	int   m_OldMode;
	CApplication() :
		m_Time(0.0f), m_SolveTime(0), m_IterationNum(20), m_Mode(eModePBD), m_OldMode(eModeMax), m_frames(0) {}

	float GetTime() { return m_Time; }
	void  SetTime(float time) { m_Time = time; }
	int   GetSolveTime() { return m_SolveTime; }
	void  SetSolveTime(float time) { m_SolveTime = time; }
	void  increaseFrames() { m_frames++; }
...
public:
	...
	void Render() {
		...
		static const glm::vec3 color(1.0f, 0.84f, 0.0f); // 球的颜色，每个通道取值范围为[0,1] RGB
	...
	void Render() {
		...
		for (int w = 0; w < m_Width - 1; w++) {
			for (int h = 0; h < m_Height - 1; h++) {
				glm::vec3 col(0.2f, 0.651f, 0.722f); // 布料的颜色1（主色）
				if (col_idx++ % 2) { col = glm::vec3(1.0f, 1.0f, 1.0f); } // 布料的颜色2（白色）
...
CCloth       g_Cloth(2.0f, 2.0f, 80, 80);
CBall        g_Ball(0.1f);
...
void display(void) {
	...
	// 显示当前绘制的是第几帧
	sprintf(debug, "FRAME NO. %ld",  g_Application.getFrames());
	std::string frame_no_text(debug);
	render_string(frame_no_text, glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT), 10, 60);
	// 显示平均每帧绘制时间
	sprintf(debug, "AVG FRAME TIME %.2f(ms)", (float)glutGet(GLUT_ELAPSED_TIME) / g_Application.getFrames());
	std::string avg_frame_time_text(debug);
	render_string(avg_frame_time_text, glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT), 10, 80);
...
void idle(void) {
	...
	dt = (dt > 0.033f) ? 0.033f : dt; // keep 30fps 避免单步模拟时间过长
